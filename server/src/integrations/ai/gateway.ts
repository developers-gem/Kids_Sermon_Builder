...
async function callOpenAIChat(body: Record<string, unknown>, maxRetries = 2): Promise<ChatResponse> {
  let attempt = 0;
  ...
  while (attempt <= maxRetries) {
    ...
    let errorData: OpenAIErrorPayload = {};
    try {
    ...
    const errorMessage = errorData?.error?.message || rawText;

    logger.error(
      { status: res.status, code: errorCode, message: errorMessage, attempt },
      "OpenAI chat request returned error",
    );

    // If quota is exhausted, no point in retrying
    if (res.status === 429 && errorCode === "insufficient_quota") {
      throw AppError.aiGenerationFailed(
        "OpenAI billing quota exceeded. Please check credits in your OpenAI dashboard.",
      );
    }
